...
void CH3() {
  ...
  mlir::MLIRContext context(registry);
  // 加载/注册方言
  mlir::north_star::NorthStarDialect* dialect =
      context.getOrLoadDialect<mlir::north_star::NorthStarDialect>();
  // 调用方言中的方法
  mlir::north_star::NSTensorType ns_tensor =
      mlir::north_star::NSTensorType::get(
          &context, {1, 2, 3}, mlir::IntegerType::get(&context, 32), 0);
  llvm::outs() << "静态NSTensor :\t";
  ns_tensor.dump();

  mlir::north_star::NSTensorType dynamic_ns_tensor =
      mlir::north_star::NSTensorType::get(
          &context, {mlir::ShapedType::kDynamic, 2, 3},
          mlir::IntegerType::get(&context, 32), 0);
  llvm::outs() << "动态NSTensor :\t";
  dynamic_ns_tensor.dump();
  llvm::outs() << "ShapedType :\t";
  mlir::ShapedType shape_type = mlir::cast<mlir::ShapedType>(dynamic_ns_tensor);
  shape_type.dump();
  llvm::outs() << "NSTensor is static:\t" << shape_type.hasStaticShape()
               << "\n";
  llvm::outs() << "NSTensor dim 1 is dynamic:\t" << shape_type.isDynamicDim(0)
               << "\n";
  llvm::outs() << "NSTensor dim 2 is dynamic:\t" << shape_type.isDynamicDim(1)
               << "\n";
}
