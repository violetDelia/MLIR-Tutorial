...
mlir::ModuleOp getModule(mlir::OpBuilder& builder) {
  ...
  builder.setInsertionPointToStart(module.getBody());
  auto f32 = mlir::Float32Type::get(context);
  auto dy_dim = mlir::ShapedType::kDynamic;
  ...
  func->setAttr(
      KDPAttrName,
      (mlir::Attribute)mlir::north_star::DataParallelismAttr::get(context, 2));

  auto block = func.addEntryBlock();
  builder.setInsertionPointToStart(block);
  // Softmax Op
  auto softmax_op = builder.create<mlir::north_star::SoftmaxOp>(
...
void CH6() {
  ...
  module->walk([](mlir::north_star::SoftmaxOp softmax) {
    if (isa<mlir::SupportedDataParallelismOp>(softmax.getOperation())) {
      llvm::outs() << "SoftmaxOp " << softmax.getOperationName()
                   << " supported data parallelism.\n";
    }
    if (softmax.supportedDistributeParallelism()) {
      llvm::outs() << "SoftmaxOp " << softmax.getOperationName()
                   << " supported distribute parallelism.\n";
    };
    if (softmax.supportedDataParallelism()) {
      llvm::outs() << "SoftmaxOp " << softmax.getOperationName()
                   << " supported data parallelism.\n";
    }
  });
